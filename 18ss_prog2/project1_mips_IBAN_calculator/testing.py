...
def get_test_sources(testPath, prefix="./"):
  return [testPath] + [prefix + 'src/' + filename for filename in [
    'validateChecksum.s',
    'iban2knr.s',
    'knr2iban.s',
    'moduloStr.s',
...
def get_bonus_test_sources(testPath, prefix="./"):
  return [testPath] + [prefix + 'src/' + filename for filename in [
    'bonus/validateChecksum.s',
    'iban2knr.s',
    'knr2iban.s',
    'moduloStr.s',
    'util.s']]
...
def runTest(testPath, dumpReference=False, prefix="./", marsPath="./mars", bonus=False):
   sources = get_test_sources(testPath, prefix) if not bonus else get_bonus_test_sources(testPath, prefix)
