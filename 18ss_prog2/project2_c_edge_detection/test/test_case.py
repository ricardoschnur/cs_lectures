...
class TestCase(object):
    def __init__(self, test_type, module, function, input_file, expected_file, name=None, verbose=VERBOSE):
        self.test_type = 'pub' if test_type == 'public' else test_type
        self.module = module
    ...
    def _get_name(self):
        return os.path.basename(self.input_file).split('.')[0] if not self.name else self.name
    ...
    def run_test(self, timeout=5):
        ...
        queue = Queue()
        p = Process(target=self._run_test, args=(queue,))
        p.start()
    ...
    def _run_test(self, queue):
        pass


class GetPixelValueTestCase(TestCase):
    def __init__(self, test_type, input_file, test_cases):
        self.test_cases = test_cases
        super(GetPixelValueTestCase, self).__init__(test_type, 'image', 'get_pixel_value', input_file, None)
    ...
    def _run_test(self, queue):
        ...
        for x, y, expected_value in self.test_cases:
            ...
            if not compare_values(actual, expected_value, SMALL_EPSILON):
                queue.put("""Incorrect result after calling get_pixel_value with:
x: {:d}
y: {:d}
img:
{}
result: {:.2f}
expected: {:.2f}""".format(x, y, input_matrix, actual, expected_value) if self.verbose else
                          'Incorrect result for get_pixel_value: expected {:.2f} but was {:.2f}.'.format(self.expected_value, actual))
                return
...
class ApplyThresholdTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        if compare_array(input_array, expected_matrix.values, SMALL_EPSILON):
            ...
            queue.put("""Incorrect result after calling apply_threshold with:
T: {:d}
img:
{}
result:
...
{}""".format(self.threshold,
             ...
             expected_matrix) if self.verbose else
                      'Incorrect result for apply_threshold.')
...
class ScaleImageTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        if compare_array(result, expected_matrix.values, LARGE_EPSILON):
            ...
            queue.put("""Incorrect result after calling scale_image with:
img:
{}
result:
{}
...
{}""".format(input_matrix,
             ...
             expected_matrix) if self.verbose else
                      'Incorrect result for scale_image.')
...
class ReadImageTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        w = ct.c_int(0)
        h = ct.c_int(0)
        filename_ptr = ct.c_char_p(self.input_file)
...
class WriteImageTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        input_matrix = matrix_from_file(self.input_file, False)
        input_array = input_matrix.get_as_c_array()
        filename_ptr = ct.c_char_p(self.filename)
...
class ConvolveTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        if compare_array(result, expected_matrix.values, LARGE_EPSILON):
            ...
            queue.put("""Incorrect result after calling convolve with:
img:
{}
kernel:
{}
...
class GradientMagnitudeTestCase(TestCase):
    ...
    def _run_test(self, queue):
        ...
        if compare_array(result, expected_matrix.values, LARGE_EPSILON):
            ...
            queue.put("""Incorrect result after calling gradient_magnitude with:
d_x:
{}
d_y:
{}
...
{}""".format(dx_matrix,
             ...
             pretty_print(result, expected_matrix.w, expected_matrix.h),
             expected_matrix) if self.verbose else
                      'Incorrect result for gradient_magnitude.')
    ...
    def _run_test(self, queue):
        ...
        def test_file(name, expected_name, tolerance):
            ...
            if error is not None:
                queue.put("Error in {}. {}".format(name, error))
                return False
            return True
