...
public class AutoPlayer_DeepSearch implements PlayerInterface {
	private int DEPTH = 6; // has to be at least 1
	...
	public MoveDirection getPlayerMove(SimulatorInterface game, UserInterface ui) {
		/*
		 * Order moves as follows (example for DEPTH = 3 and NUM_DIR = 4): 0: (0,0,0) 1:
		 * (0,0,1) 2: (0,0,2) 3: (0,0,3) 4: (0,1,0) ... 15: (0,3,3) 16: (1,0,0) ... 63:
		 * (3,3,3)
		 *
		 * Here the i-th element of the tuple stands for the direction in this turn,
		 * with 0 <> NORTH 1 <> EAST 2 <> SOUTH 3 <> WEST
		 * 
		 * Note that in general the pair (i_0, ..., i_{DEPTH-1}) appears at position 0
		 * <= p < NUM_DIR^DEPTH, where p = i_0 * NUM_DIR^{DEPTH-1} + i_1 *
		 * NUM_DIR^{DEPTH-2} + ... + i_{DEPTH-1} * NUM_DIR^0
		 * 
		 * Conversely, given a position p, you can get back the direction i_0 of the
		 * first move via integer division i_0 = p / NUM_DIR^{DEPTH-1}
		 */
	...
	private void searchDepth(SimulatorInterface currentGame, int[] scores, boolean[] isPossible, int currentDepth,
			int currentPos, int[] powers) {
			...
			if (game.performMove(direction)) {
				...
				if (copy1.performMove(MoveDirection.NORTH)) {
					if (!copy1.isMovePossible()) {
						...
						for (int j = 0; j < 4; j++) {
							addScore(scores, depth, pos, powers, j,
									-1 * java.lang.Math.max(MAX_PENALTY, game.getPoints()));
						}
					}
				}
	...
	private void setImpossible(boolean[] isPossible, int currentDepth, int currentPos, int[] powers, int dir) {
		int pos = currentPos + dir * powers[DEPTH - 1 - currentDepth];
		int range = pos + powers[DEPTH - 1 - currentDepth];
		for (; pos < range; pos++) {
