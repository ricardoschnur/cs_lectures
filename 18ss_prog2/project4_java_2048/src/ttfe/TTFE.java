...
public class TTFE {
	 ...
	 * game is started the arguments (if any) are read and a Simulator as well as
	 * the UserInterface and the Player object is created. The control is then
	 * transfered to the Simulator that should run until the game is over.
	 *
	 * @param args
	 *            The command line arguments provided when the program was executed.
	 *            The command line arguments are expected to be in the following
	 *            order and only accepted this way. However, it is possible to omit
	 *            trailing arguments, hence to provide only the first N instead of
	 *            all.
	 *
	 *            No. Meaning Possible Values
	 *
	 *            (1) random-seed (int) (2) board-width (int) (3) board-heigh (int)
...
//		}
	...
	/**
	 * @param str
	 *            The string that should be converted to an boolean.
	...
	/**
	 * @param str
	 *            The string that should be converted to an integer
	 * @return The integer value represented by str or def if str does not represent
	 *         an integer.
	 */
